...
def aggregate_by_date(list_of_timestamps):
    ...
    for timestamp in list_of_timestamps:
        date = datetime.datetime.fromtimestamp(int(timestamp/1000)).strftime('%d/%m/%Y')
        if date in output:
            output[date]+=1
        else:
            output[date]=1
...
def count_views_LnD(list_of_timestamps,n):
    ...
    output=[0]*n
    for timestamp in list_of_timestamps:
        dt = datetime.datetime.fromtimestamp(timestamp/1000.0)  #convert js timestamp (ms) to python timestamp (s)
...
re.sub(r'\\textbf\{(.*?)\}', r'<b>\1<\/b>', "This is an [[example]] sentence. It is [[awesome]]");
